import { c as _defineProperty, i as _typeof, _ as _inherits, a as _createSuper, e as _createClass, b as _classCallCheck, d as _assertThisInitialized, h as _objectSpread2, f as _slicedToArray } from './_rollupPluginBabelHelpers-cc1db274.js';
...
const CONFIG = {
  /**@type {string} 默认服务端地址key，将映射到 host 中*/
...
function getHost(option) {
  return objectHas(option, 'host') && option.host ? option.host : CONFIG.host || CONFIG.defaultHost;
}
/**
 * 合并请求的配置
...
function request(option, param) {
  ...
  for (const key in OPTION) {
    if (objectHas(OPTION, key)) {
      opt[key] = objectHas(option, key) ? option[key] : OPTION[key];
    }
  }
  ...
  if (option.onBefore(opt, controller) === false) {
    return Promise.reject(error('The request has been canceled.')).catch(err => {
      option.onError(err);
      throw err;
    });
  }

  const onProgress = isDownload ? getProgress(option) : fakeProgress(option.contentLength, option.onProgress, option.progressDelay);

  const fakeProgressEnd = () => {
    isDownload ? void 0 : onProgress.end();
  };
  ...
  _createClass(InputFile, [{
    ...
    value: function loadView(isCustomized, key) {
      ...
      if (isCustomized === false) {
        ...
        switch (key) {
          case 'picture':
            render = import('./Picture-b3de2c47.js');
            break;

          case 'dragbox':
            render = import('./Dragbox-48e51e12.js');
            break;

          case 'list':
          default:
            render = import('./List-b278f2d5.js');
            break;
        }
